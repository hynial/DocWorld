...
public class PoiCopySheet {
    ...
    private void copySheetInst() {
        ...
        // 复制数据验证
        for (DataValidation validation : srcSheet_.getDataValidations()) {
            ...
            for (CellRangeAddress crAddress : crAddresses)
                addrList.addCellRangeAddress(crAddress.getFirstRow(), crAddress.getFirstColumn(), crAddress.getLastRow(), crAddress.getLastColumn());

            ExcelUtil.copyDataValidationRegion(srcSheet_, dstSheet_, validation.getRegions(), addrList);
        }
    }
...
//        CellStyle dstCellStyle = getSameCellStyle(srcCellStyle); // 省事反而多了样式的差异 - 无法拷贝编辑状态下的背景
...
//            dstCellStyle.cloneStyleFrom(srcCellStyle);
    ...
    private CellStyle cloneCellStyle(CellStyle srcCellStyle) {
        ...
        dstCellStyle.cloneStyleFrom(srcCellStyle);  // 少了字体
        dstCellStyle.setFont(dstFont);              // 补上字体
